 ...
 */
 ...
 $this->getOutput()->addHTML("</select></td></tr>
<tr><td><input type='submit' value='Search' name=searchtr> </td></tr></table></form>");
 if(isset($_POST['searcht'])){


	$res = $dbr->select(
		array( 'material',$_POST['searcht']),
		array( 'material_name','value',"{$dbr->tableName( $_POST['searcht'] )}.timestamp" ),
		array(
			'mat_id>0'
		),
		__METHOD__,
	...
	$this->getOutput()->addHTML("<table border='1' width='450' height='30' cellspacing='1' cellpadding='3'><tr><th>Material Name</th><th>".ucwords(str_ireplace("_", " ", $_POST['searcht']))."</th><th>Timestamp</th></tr>");
...
$f=0;
 ...
 $array1[$f] = $samedata->material_name;
 $f++;
 }
 
$valuebp=$dbr->select('boiling_point',array('value'),"",__METHOD__);
$h=0;
foreach($valuebp as $samedata){
	$arraybp[$h] = $samedata->value;
	$h++;
}
//print_r($array1);
//print_r($arraybp);
$combine=array_combine($array1,$arraybp);
//print_r($combine);
